...
class EventsController < ApplicationController
  ...
  before_action only: %i[index index_day] do
    confirm_logged_in('Please Log In') unless user_signed_in?
  end
  before_action :confirm_staff, only: %i[new create edit update delete destroy]
  ...
  def index_day
    ...
    if current_user.check_staff?
      redirect_to new_event_path(start_date: date),
                  alert: "No events for #{date.strftime('%B %d, %Y')}. Redirected to Event Creation"
    else
  ...
  def event_params
    if current_user.check_executive?
      params.require(:event).permit(:name, :committee_id, :start_timestamp, :end_timestamp, :location, :point_value,
                                    :passcode)
    else
